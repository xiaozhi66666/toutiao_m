...
<!-- 头部E -->
        <van-popup v-model="showPopup" position="bottom" :style="{ height: '100%' }" duration="3s" >
          <div class="popup">   <header><van-nav-bar
                left-text="返回"
                left-arrow
                @click-left="isBack"
                ><span slot="title">{{aplyList.length}}条回复</span></van-nav-bar></header>
          <main>
             <div >
                ...
                <span slot="default" class="icon" >
                  <span class="toutiao toutiao-dianzan1" v-if="contentList.is_liking" @click="changeLiking(contentList.com_id,contentList.is_liking)"></span>
                <span class="toutiao toutiao-dianzan" style="margin-right:10px" v-else @click="changeLiking(contentList.com_id,contentList.is_liking)"></span>赞
                </span>
                </van-cell>
                <div class="comment-content-time">
                    <div class="comment-content">{{contentList.content}}</div>
                <div  class="comment-time">{{contentList.pubdate| timeFormat}}<span>
                    <van-button round>回复 {{contentList.reply_count}}</van-button>
                    </span>
                </div>
            </div>
            </div>
            ...
            <van-list
                ...
                >
                  <div class="content-all" v-for="item,index in aplyList" :key="index">
                <div class="comment"  >
                <van-cell >
                <div slot="title" class="author-focus"><van-image
                width="50"
                ...
                <span slot="default" class="icon" >
                <span class="toutiao toutiao-dianzan1 active" @click="toLike(item.com_id,item.is_liking)" v-if="item.is_liking"></span>
...
<script>
import { setCommentsAPI, getCommentsAplyAPI, setCommentLikeAPI, delCommentLikeAPI } from '@/api'
import stroage from '@/utils/stroage'
export default {
  name: 'ToutiaoMPopup',
  props: {
    showPopup: { // 从外部接收的是否展示弹出层的值
      type: Boolean
    },
    contentList: { // 接收的回复对象数据
      type: Object
  ...
  methods: {
    ...
    async goPublish () {
      ...
      try {
        const res = await setCommentsAPI(this.idAply, this.articleComment, this.artId)
    ...
    async onLoad () {
      if (this.offset) {
        try {
          const res = await getCommentsAplyAPI('c', this.idAply, this.offset, this.limit)
